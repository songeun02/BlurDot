...
class SynthTextDataset(data.Dataset):
	def __init__(self, cfg, target_size=(256, 256)):
		...
		self.cfg = cfg
		self.img_path = cfg.synthtext_img_path  # PNG 이미지가 있는 폴더 경로
		self.gt_path = cfg.synthtext_gt_path    # 텍스트 파일 경로 (하나의 파일)
		self.target_size = target_size
		
		# 텍스트 파일에서 이미지와 좌표 정보 읽기
		self.image_data = self._load_gt_file()
	...
	def _load_gt_file(self):
		"""Ground truth 파일에서 이미지와 좌표 정보를 읽어옴"""
		image_data = []
		...
		for line in lines:
			parts = line.strip().split(',')
			image_name = parts[0]  # 이미지 파일 이름
			coordinates = list(map(float, parts[1:]))  # 좌표값
			...
			image_data.append({
                ...
                "coordinates": np.array(coordinates).reshape((-1, 2))  # (n, 2) 형태로 변환
            })
	...
	def __getitem__(self, index):
		...
		# region_score 및 affinity_score는 모델에 맞게 생성 (사용자 정의 함수 필요)
		region_score, affinity_score = self.generate_score_maps(coordinates)
	...
	def generate_score_maps(self, coordinates):
		"""
