...
from S_detect import resize_img, get_score
...
# 		self.cfg = cfg
# 		self.img_path = cfg.synthtext_img_path  # PNG 이미지가 있는 폴더 경로
# 		self.gt_path = cfg.synthtext_gt_path      # 텍스트 바운딩 박스 정보가 있는 텍스트 파일 폴더 경로
        
#         # 이미지 파일 목록 가져오기
# 		self.img_files = [f for f in os.listdir(self.img_path) if f.endswith('.png')]
...
# 	def _parse_gt_file(self, gt_file_path):
# 		wordBB = []
# 		words = []
...
# 			filename = parts[0]  # 이미지 파일명
# 			coordinates = list(map(float, parts[1:]))  # 좌표들
			
# 			if len(coordinates) != 8:
# 				continue  # 잘못된 형식은 건너뜀
			
# 			# 좌표를 (2, 4) 형태로 변환
# 			word_coords = np.array(coordinates).reshape((4, 2)).transpose((1, 0))
# 			wordBB.append(word_coords)
# 			words.append(filename)  # 파일명을 단어 텍스트로 간주
			
# 		return np.array(wordBB).transpose((1, 2, 0)), words
...
#         # 해당 이미지의 텍스트 파일 찾기 (확장자는 상황에 맞게 수정)
# 		gt_file = img_file.replace('.png', '.txt')
...
#         # 바운딩 박스 정보 로드
# 		wordBB, words = self._parse_gt_file(gt_path)
        
#         # CRAFT 모델을 사용해 문자 바운딩 박스 탐지
# 		charBB = self.detect_char_boxes(img, wordBB, self.model, self.device)
        
#         # 데이터 증강 및 전처리
# 		img, wordBB, charBB = self.augment_data(img, wordBB, charBB)
        
#         # 점수 맵 생성
# 		region_score, affinity_score = self.generate_score_maps(wordBB, charBB, words)
...
# 		return transform(img), region_score, affinity_score

class SynthTextDataset(data.Dataset):
	def __init__(self, cfg, target_size=(256, 256)):
		super(SynthTextDataset, self).__init__()
		self.cfg = cfg
		self.img_root = cfg.synthtext_img_root  # 최상위 이미지 폴더 경로
		self.gt_root = cfg.synthtext_gt_root    # 최상위 텍스트 파일 폴더 경로
		self.target_size = target_size

		# 이미지 및 텍스트 파일 목록 가져오기
		self.image_data = self._load_image_and_gt_files()

		# 데이터 증강 및 전처리에 사용할 변환 정의
	...
	def _load_image_and_gt_files(self):
		"""이미지 및 Ground Truth 텍스트 파일 경로를 수집."""
		image_data = []

		# 이미지 폴더 탐색
		for img_subfolder in os.listdir(self.img_root):
			img_folder_path = os.path.join(self.img_root, img_subfolder)

			if not os.path.isdir(img_folder_path):
				print(f"Skipping: {img_folder_path} is not a directory.")
				continue

			# 이미지 폴더 이름에서 정답 폴더 이름을 추출 (tr_img1-1000 -> tr_gt1-1000)
			gt_subfolder = img_subfolder.replace('tr_img', 'tr_gt')
			gt_folder_path = os.path.join(self.gt_root, gt_subfolder)

			if not os.path.isdir(gt_folder_path):
				print(f"Skipping: {gt_folder_path} is not a directory.")
				continue  # 대응하는 Ground Truth 폴더가 없으면 건너뜀

			for img_file in os.listdir(img_folder_path):
				if not img_file.endswith(('.png', '.jpg', '.jpeg','gif')):
					print('not_found',img_file)
					continue

				# 이미지 파일 이름에 맞는 Ground Truth 파일 경로 생성
				img_path = os.path.join(img_folder_path, img_file)
				gt_file = img_file.rsplit('.', 1)[0] + '.txt'  # 확장자 .png 또는 .jpg -> .txt로 변경
				gt_path = os.path.join(gt_folder_path, gt_file)

				if os.path.exists(gt_path):
					image_data.append({"image_path": img_path, "gt_path": gt_path})
				else:
					print(f"Ground Truth file not found for: {img_path}")

		print(f"Collected {len(image_data)} image files.")
	...
	def _parse_gt_file(self, gt_file_path):
		...
		for line in lines:
			...
			coordinates.append(np.array(coords).reshape((4, 2)))  # 4x2 형태로 변환
			
		return np.array(coordinates) if coordinates else np.zeros((0, 4, 2))
	...
	def __getitem__(self, index):
		...
		# Ground Truth 로드
		wordBB = self._parse_gt_file(gt_path)  # (n, 4, 2)

		if self.transform:
			img = self.transform(img)

		# region_score 및 affinity_score는 사용자 정의 함수로 생성
		region_score, affinity_score = self.generate_score_maps(wordBB)

		conf_map = np.ones(self.target_size)

		return img, region_score, affinity_score, conf_map

	def generate_score_maps(self, wordBB):
		"""
		region_score와 affinity_score를 생성.
		사용자의 점수 맵 생성 로직에 따라 수정.
		"""
		# 예시로 빈 점수 맵 반환 (사용자의 논리에 맞게 구현)
		region_score = np.zeros(self.target_size)  # 임의 크기
		affinity_score = np.zeros(self.target_size)
